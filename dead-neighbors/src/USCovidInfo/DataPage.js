...
class DataPage extends React.Component {
  constructor() {
    ...
    this.windowClickPosition = this.windowClickPosition.bind(this);
    this.menuClickPosition = this.menuClickPosition.bind(this);
  }
  componentDidMount() {
    /*event listeners to menu and window */
    const menu = document.querySelector(".menu-wrapper");
    menu.addEventListener("click", this.menuClickPosition);
    window.addEventListener("click", this.windowClickPosition);
  }
  componentWillUnmount() {
    const menu = document.querySelector(".menu-wrapper");
    window.removeEventListener("click", this.windowClickPosition);
    menu.removeEventListener("click", this.menuClickPosition);
  }
  windowClickPosition = (e) => {
    /*sets x and y of click to state, if it does not match menu x and y, the menu will close */
    this.setState({ windowX: e.screenX, windowY: e.screenY });
    if (
      this.state.windowX !== this.state.menuX &&
      this.state.windowY !== this.state.menuY
    ) {
      this.setState({ showMenu: false });
    }
  };
  menuClickPosition = (e) => {
    /*sets x and y of click to state*/
    this.setState({ menuX: e.screenX, menuY: e.screenY });
  };
  scrollToTop = () => {
    window.scrollTo(0, 0);
  };
  ...
  slideHandler = () => {
    /* sets true/false US or State display */
    this.setState({ slider: !this.state.slider });
    this.scrollToTop();
  };
  filterView = () => {
    /* sets true/false for filters visible */
    this.setState({ showFilter: !this.state.showFilter });
  };
  menuView = () => {
    /* sets true/false for menu open/close */
  ...
  filterSortView = (event) => {
    ...
    }); /*click event value from sorting options */
  ...
  viewHandler = (data) => {
    ...
    const CovidStateData = [
      ...CovStateData,
    ]; /*spread operator to copy props to new array */
    ...
    if (dataView === "Death Increase Daily - High To Low") {
      ...
      /*does not take in data parameter, uses the spread operater arrays*/
      ...
      let stateChecker =
        ""; /*used to see if next object has the same state as the current object */
      let worstDaysByState = []; /* 1st result of each state is pushed to worstDaysByState array */
      CovidStateData.forEach((data, i) => {
        if (i === 0) {
          /*first state result is the first worst day after sorting operations and pushed to worstDaysByState variable */
          stateChecker = data.state;
          return worstDaysByState.push(data);
        } else if (data.state !== stateChecker) {
          /* if object state is different, object is pushed to worstDaysByState variable */
          stateChecker = data.state;
          return worstDaysByState.push(data);
        }
      });
      worstDaysByState.sort(
        (a, b) => b.deathIncrease - a.deathIncrease
      ); /*sorts worst days once forEach Loop is complete */
      return worstDaysByState;
    } else {
      /* if no match returns original data */
      return dataArray;
    }
  };
  ...
  render() {
    ...
    let radioFilters = [
      /* set value of radio button filters */
    ...
    let filterArrow = (
      /* create arrows for menu up/down visualization*/
      <svg viewBox="10 45 100 120" height="20px" width="35px">
        <polyline points="0 60 60 145 120 60" />
      </svg>
    );
    return (
      <main className="dataPage">
        ...
        <div className="menu-wrapper">
          <div
            className={!this.state.showMenu ? "data-menu up" : "data-menu down"}
            onClick={this.menuView}
          >
            {" "}
            {/* show/ hide menu tab */}
            <div
              className={
                !this.state.showMenu ? "arrow up one" : "arrow down one"
              }
              id="menu"
            >
              {filterArrow}
            ...
            <div
              className={
                !this.state.showMenu ? "arrow up two" : "arrow down two"
              }
              id="menu"
            >
              {filterArrow}
          ...
          <div
            className={
              !this.state.showMenu ? "slider-filters up" : "slider-filters down"
            }
          >
            {" "}
            {/* slider/sorting options menu */}
            <div className="slider-filter-wrapper">
              <Slider boolean={this.slideHandler} usStateBoolean={slider} />
              {/* slider to choose US or State Info display */}
              <div
                className={!showFilter ? "filter-sort up" : "filter-sort down"}
                onClick={this.filterView}
              >
                {/* show/hide sorting tab */}
                <div
                  className={!showFilter ? "arrow up one" : "arrow down one"}
                  id="sort"
                >
                  {filterArrow}
                </div>
                ...
                <div
                  className={!showFilter ? "arrow up two" : "arrow down two"}
                  id="sort"
                >
                  {filterArrow}
                </div>
        ...
        <div className="infoSection">
          {!slider /* state/us view */ ? (
            <>
              {this.state.dataView !== "Worst Day - By State" ? (
            ...
            <div className="usStats">
              <CovidUsDataComplete
                data={this.viewHandler(
                  CovUsData
                )} /* renders usData and sorts with viewHandler */
              />
            </div>
          )}
        </div>
